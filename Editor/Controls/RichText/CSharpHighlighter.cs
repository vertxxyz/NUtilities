...
namespace Vertx
{
	public class CSharpHighlighter
	{
		...
		public const string INDEX_PREFIX = "¤";
		...
		protected virtual string HighlightSource(string content)
		{
			...
			regex = new Regex($@"(?<![\w{INDEX_PREFIX}])([0-9]+)(?!\w)", RegexOptions.Singleline);
			...
			for (int i = 0; i < comments.Count; i++)
			{
				...
				for (int q = 0; q < quotes.Count; q++)
					comment = comment.Replace($"{QUOTES_TOKEN}{INDEX_PREFIX}{q}{QUOTES_TOKEN}", quotes[q]);
	...
	static class MoreExtensions
	{
		...
		public static string ReplaceTokenWithCss(this string content, string source, string token, int counter, string cssClass) => content.Replace($"{token}{CSharpHighlighter.INDEX_PREFIX}{counter}{token}", $"<span class=\"{cssClass}\">{source}</span>");

		public static string ReplaceToken(this string content, string source, string token, int counter) => content.Replace(source, $"{token}{CSharpHighlighter.INDEX_PREFIX}{counter}{token}");
	}
}